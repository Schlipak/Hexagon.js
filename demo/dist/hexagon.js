...
(function() {
	...
	var Cursor = function(args) {
		...
		this.draw = function() {
			...
			this.angle = (this.angle + (this.dir * this.speed)) % 360;
	...
	Hexagon = function(args) {
		...
		var _update = function() {
			...
			this.ctx.translate(- this.canvas.width / 2, - this.canvas.height / 2);
			this.rays.draw(this.canvas, ["#000", "#082410"]);
			for (var i = 0; i < this.walls.length; i++) {
				this.walls[i].draw(this.canvas, "#33E566");
			...
			this.hexagon.draw(this.canvas, "#000", "#33E566", 7);
			this.cursor.draw();
